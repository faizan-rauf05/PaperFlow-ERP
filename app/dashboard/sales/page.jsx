...
export default function SalesDashboardPage() {
  ...
  const [newClicheForm, setNewClicheForm] = useState({
    widthMm: "",
    heightMm: "",
    colorCount: "",
  ...
  function openInspectOrder(order) {
  ...
  async function handleRecordCustomerResponse() {
    ...
    try {
      const { data } = await api.post(`/orders/${inspectOrder.id}/customer-approval`, {
      ...
      try {
        const { data } = await api.get("/cliches", {
          params: {
            ...
            customerId: inspectOrder?.customerId || undefined,
          },
        });
  ...
  async function handleAssignExistingCliche(clicheId) {
    ...
    try {
      ...
      const refreshed = (data.orders || []).find((o) => o.id === inspectOrder.id);
      if (refreshed) applyOrderUpdate(refreshed);
  ...
  async function handleSendToProduction() {
    ...
    try {
      const { data } = await api.post(`/orders/${inspectOrder.id}/send-to-production`);
      applyOrderUpdate(data.order);
          ...
          {inspectOrder && (
            <>
              ...
              <div className="space-y-4 py-3">
                ...
                {Array.isArray(inspectOrder.quoteApprovals) && inspectOrder.quoteApprovals.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-xs text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                    ...
                    <div className="border rounded-md p-3 space-y-3 text-xs">
                      {inspectOrder.quoteApprovals.map((q, qIdx) => (
                      ...
                      {inspectOrder.status === "PENDING_CUSTOMER_APPROVAL" && (
                ...
                {/* Cliche Assignment */}
                {["CUSTOMER_APPROVED", "READY_FOR_WORK", "PICKED", "IN_PROGRESS", "COMPLETED"].includes(inspectOrder.status) && (
                  <div>
                    <h4 className="font-semibold text-xs text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                      ...
                      {(inspectOrder.lines || []).map((l, i) => (
                        <div key={l.id || i} className="p-3 text-xs flex items-center justify-between gap-3">
                          ...
                          <Button
                            ...
                            disabled={inspectOrder.status !== "CUSTOMER_APPROVED"}
                          >
                    ...
                    {inspectOrder.status === "CUSTOMER_APPROVED" && (
                      <div className="mt-3">
                        {(() => {
                          const missing = (inspectOrder.lines || []).filter((l) => !l.clicheId);
                          return missing.length > 0 ? (
                            <p className="text-xs text-amber-700 dark:text-amber-400 mb-2">
                              {missing.length} of {inspectOrder.lines.length} line(s) still need a cliche before this can go to production.
                            </p>
                          ) : null;
                        })()}
                        <Button
                          size="sm"
                          className="w-full"
                          disabled={sendingToProduction || (inspectOrder.lines || []).some((l) => !l.clicheId)}
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="flex-1"
                  disabled={assigningCliche}
                  onClick={handleCreateAndAssignCliche}
                >
