import { z } from "zod";
import { resolveInkColor } from "@/lib/material-code";
...
// Cost price is optional (older materials predate this field, and an edit
// unrelated to cost shouldn't be blocked by it), but whenever a value IS
// given it must be a plain positive number with at most 2 decimal places.
// Checked as a string first so "45.999" is rejected instead of silently
// rounded — z.coerce.number() alone can't tell 2dp from 3dp+ input.
const costPriceAmountField = z
  ...
  .superRefine((v, ctx) => {
    if (v === "") return;
    if (!/^\d+(\.\d{1,2})?$/.test(v)) {
      const message = /^\d+\.\d+$/.test(v)
        ? "Cost price can have at most 2 decimal places"
    ...
    if (num <= 0) {
      ...
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Cost price is too large" });
    }
  })
  .transform((v) => (v === "" ? null : Number(v)));

const costPriceFields = {
  costPriceAmount: costPriceAmountField,
  costPriceCurrency: z.enum(["KWD", "USD"]).optional().default("KWD"),
  costPriceEntryBasis: z.enum(["PER_UNIT", "PER_PACK"]).optional().default("PER_UNIT"),
};
...
const paperRollMaterialSchema = z.object({
  ...
  barCode: z.string().trim().min(3, "Scan or enter the roll's barcode"),
  ...costPriceFields,
});
...
const PACK_PRICEABLE_TYPES = ["PAPER_ROLL", "GLUE", "INK", "ROPE", "CARTON"];
...
export const materialSchema = z
  ...
  .superRefine((data, ctx) => {
    if (
      data.costPriceEntryBasis === "PER_PACK" &&
      !PACK_PRICEABLE_TYPES.includes(data.materialType)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Per-pack pricing isn't available for this material type",
        path: ["costPriceEntryBasis"],
      });
    }

    if (["GLUE", "INK", "ROPE"].includes(data.materialType) && data.batchNo?.trim()) {
      if (!data.receivingDate) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "Enter the batch date (needed to keep this batch number unique)",
          path: ["receivingDate"],
        });
      }
    }
