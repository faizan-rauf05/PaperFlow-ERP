...
const paperRollMaterialSchema = z.object({
  ...
  paperType: z.enum(["RECYCLED", "VIRGIN"], { errorMap: () => ({ message: "Select paper type" }) }),
  paperColor: z.enum(["BROWN", "WHITE"], { errorMap: () => ({ message: "Select paper color" }) }),
...
const kaptonMaterialSchema = z.object({
  ...
  supplier: supplierField,
  codeSuffix: codeSuffixField,
  tapeType: z.enum(["FLEXO", "WHITE_LIGHT_DS", "CARTOON", "MACHINE_BLACK_DUCK"], { errorMap: () => ({ message: "Select Tape type" }) }),
