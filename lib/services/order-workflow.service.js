...
export async function reviewOrderApproval({
  ...
  const nextStatus = isApprove ? "PENDING_CUSTOMER_APPROVAL" : "REJECTED";
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (reviewedById) {
      await writeAuditLog({
        userId: reviewedById,
        action: isApprove ? ACTIONS.QUOTE_SENT : "ORDER_REJECTED",
        model: "ProductionOrder",
        recordId: orderId,
        newValue: {
          status: nextStatus,
...
export async function recordCustomerQuoteResponse({
  ...
  const order = await prisma.productionOrder.findUnique({
    where: { id: orderId },
    include: { quoteApprovals: { orderBy: { sentAt: "desc" } } },
  });
  if (!order) throw new Error("Order not found");
