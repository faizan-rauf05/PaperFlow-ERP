...
import { convertQuantity, computeBagsPerMeter } from "@/lib/services/unit-conversion.service";
import {
  computePlannedGlue,
...
async function resolveBagSpecId(db, line) {
  if (line.bagSpecId) {
    const existing = await db.bagSpecification.findUnique({ where: { id: line.bagSpecId } });
    if (!existing) throw new Error(`Bag specification not found: ${line.bagSpecId}`);
    return existing.id;
  }

  const sizeLabel = String(line.bagSize || "").trim();
  if (!sizeLabel) throw new Error("Each line needs a bag size or bag specification");
...
export async function createProductionOrder({ customerId, assignedWorkerId, notes, lines }) {
  ...
  for (const line of lines) {
    if (!line.bagSpecId && !String(line.bagSize || "").trim()) {
      throw new Error("Each line needs a bag size");
    }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      const bagSpecId = await resolveBagSpecId(tx, line);
...
async function getStageInOrder(orderId, stageId) {
  const stage = await prisma.productionStage.findFirst({
    ...
    include: {
      orderLine: { include: { bagSpec: true } },
      material: true,
      qcRecords: true,
    },
  });
...
export async function recordStage({
  ...
  if (!["READY", "IN_PROGRESS"].includes(stage.status) && stage.status !== "COMPLETED") {
    // Allow re-record only if READY or IN_PROGRESS; COMPLETED is preview-only unless we allow overwrite
    if (stage.status === "COMPLETED") throw new Error("Stage already completed — use Preview");
  ...
  if (proofs.length === 0) throw new Error("At least one proof image is required");
  ...
  if (isQc) {
    ...
    if (!cutWidthMm || Number(cutWidthMm) <= 0) throw new Error("Cut width is required");
    ...
    resolvedPieceCount = parentWidth > 0 ? Math.floor(parentWidth / cutW) : resolvedPieceCount;
    ...
    if (lengthRestock.lt(0)) throw new Error("Length restock cannot be negative");
    if (lengthRestock.gt(grossUsable)) throw new Error("Length restock cannot exceed usable cut meters");

    output = grossUsable.sub(lengthRestock);
    const widthRemMm = parentWidth > 0 ? parentWidth - resolvedPieceCount * cutW : 0;
    ...
    if (output.gt(inputQty)) throw new Error("Printed meters cannot exceed input");
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const updatedStage = await tx.productionStage.update({
      ...
      data: {
        ...
        remainderQty: stage.stageType === "SLITTING" ? widthRemainderQty : (remainderQty != null ? dec(remainderQty) : null),
  ...
  if (stage.stageType === "SLITTING") {
    const paperId = materialForStage || (await prisma.productionStage.findUnique({
      where: { orderLineId_sequence: { orderLineId: stage.orderLineId, sequence: 1 } },
    }))?.materialId;
  ...
  if (stage.stageType === "HANDLE_MAKING_PASTING") {
    ...
    const actualBottom = glueBottomQty != null ? dec(glueBottomQty) : plannedBottom;
...
export async function getStageRecordContext(orderId, stageId) {
  ...
  let handleCapacity = null;
  let gluePlan = null;
  if (stage.stageType === "HANDLE_MAKING_PASTING") {
    handleCapacity = await getHandleCapacity({
      bagSpec,
      metersAvailable: inputQty,
    });
    const estBags = handleCapacity?.capacityBags ?? 0;
    gluePlan = {
      sideKg: Number(computePlannedGlue({ bagSpec, consumptionKind: "GLUE_SIDE", bagCount: estBags })),
      bottomKg: Number(computePlannedGlue({ bagSpec, consumptionKind: "GLUE_BOTTOM", bagCount: estBags })),
      ropePcs: Number(computePlannedHandleRope({
        bagCount: estBags,
        handlesPerBag: bagSpec?.handlesPerBag ?? 2,
      })),
      bagsPerMeter: bagSpec?.bagsPerMeter != null ? Number(bagSpec.bagsPerMeter) : null,
      handlesPerBag: bagSpec?.handlesPerBag != null ? Number(bagSpec.handlesPerBag) : 2,
    };
  }
  ...
  return {
    ...
    handleCapacity,
    gluePlan,
    suggestedCutWidthMm: bagSpec?.bagWidthMm != null ? Number(bagSpec.bagWidthMm) : null,
  };
}
...
export async function getWorkerTasks(workerId) {
  return prisma.productionStage.findMany({
    ...
    include: {
      orderLine: { include: { bagSpec: true, order: { include: { customer: true } } } },
      machine: true,
      material: true,
    },
    orderBy: [{ sequence: "asc" }],
...
export async function getManagerKpis() {
  const [runningOrders, readyStages, lowStockCount] = await Promise.all([
    ...
    (async () => {
      const stocks = await (await import("@/lib/services/inventory.service")).getAllMaterialStock();
