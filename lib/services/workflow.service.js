...
export async function recordStage({
  ...
  if (isQc) {
    ...
    if (passed.add(rejected).gt(inputQty) && inputQty.gt(0)) {
    ...
    // Output meters = usable length after cut (operator confirms); remainder handled separately
    if (output.lte(0)) throw new Error("Output length is required");
    const rem = remainderQty != null ? dec(remainderQty) : inputQty.sub(output);
    if (rem.lt(0)) throw new Error("Remainder cannot be negative");
    if (rem.gt(0) && !remainderAction) {
      throw new Error("Choose Waste or Restock for remaining length");
    }
    waste = remainderAction === "WASTE" ? rem : dec(0);
    ...
    // waste meters = leftover input not converted (optional estimate via bagsPerMeter)
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const updatedStage = await tx.productionStage.update({
      ...
      data: {
        ...
        pieceCount: pieceCount != null ? parseInt(pieceCount, 10) : null,
        pieceWeightKg: pieceWeightKg != null ? dec(pieceWeightKg) : null,
        remainderAction: remainderAction || null,
        remainderQty: remainderQty != null ? dec(remainderQty) : (stage.stageType === "SLITTING" ? inputQty.sub(output) : null),
      },
    });
    ...
    if (rem.gt(0) && paperId) {
      if (remainderAction === "WASTE") {
        await postInventoryTransaction({
          ...
          unit: "METER",
          referenceId: stageId,
          remarks: "Slitting remainder waste",
          createdById: userId,
        });
        ...
        await postInventoryTransaction({
          ...
          unit: "METER",
          referenceId: stageId,
          remarks: "Slitting remainder restock",
          createdById: userId,
        });
      }
    }
  ...
  if (stage.stageType === "HANDLE_MAKING_PASTING") {
    ...
    if (plannedBottom.gt(0)) {
      await recordStageConsumption({
        stageId,
        consumptionKind: "GLUE_BOTTOM",
        plannedQty: plannedBottom,
        actualQty: plannedBottom,
        unit: "KG",
        workerId: userId,
      });
    }
