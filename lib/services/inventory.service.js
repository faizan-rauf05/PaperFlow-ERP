...
export async function postInventoryTransaction({
  ...
  // Soft check only for now: a STOCK_OUT/WASTE that would take stock below zero
  // is still allowed (not blocked) but flagged, so callers can warn the user.
  // Turning this into a hard block is a later, separate decision.
  let warning = null;
  if (STOCK_OUT_TYPES.includes(transactionType)) {
    const currentStock = await getMaterialStock(materialId);
    if (currentStock.sub(qty).lessThan(0)) {
      warning = `This posts ${material.name} below zero stock (available: ${currentStock.toString()}).`;
    }
  }
