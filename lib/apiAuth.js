...
export async function requireWarehouse() {
  ...
  if (authResult.error) return authResult;
  const role = authResult.session.user.role;
  if (!["WAREHOUSE", "MANAGER", "ADMIN"].includes(role)) {
    return { error: { status: 403, body: { error: "Forbidden" } } };
  }
  return authResult;
}

export async function requireWorkerOrWarehouse() {
  const authResult = await requireAuth();
  if (authResult.error) return authResult;
  const role = authResult.session.user.role;
  if (!["WORKER", "WAREHOUSE", "MANAGER", "ADMIN"].includes(role)) {
    return { error: { status: 403, body: { error: "Forbidden" } } };
  }
  return authResult;
}

export async function requireSalesOrAbove() {
  const authResult = await requireAuth();
  if (authResult.error) return authResult;
  const role = authResult.session.user.role;
