...
export function generateMaterialCode(input) {
  ...
  if (!base) return "";
  const suffix = (input.codeSuffix || "").trim().toUpperCase() || createCodeSuffix();
  return `${base}-${suffix}`;
}
...
export function generatePaperRollBarcode(input) {
  const typeCode = input.paperType === "RECYCLED" ? "REC" : input.paperType === "VIRGIN" ? "VRG" : "PR";
  const colorCode = input.paperColor === "BROWN" ? "BRN" : input.paperColor === "WHITE" ? "WHT" : "CLR";
...
export function buildMaterialRecord(input) {
  ...
  if (materialType === "PAPER_ROLL") {
    const pType = input.paperType === "RECYCLED" ? "Recycled" : input.paperType === "VIRGIN" ? "Virgin" : "Paper";
    const pColor = input.paperColor === "BROWN" ? "Brown" : input.paperColor === "WHITE" ? "White" : "";
    name = `${pType} ${pColor} Paper Roll ${input.paperWidthMm ? input.paperWidthMm + "mm" : ""} ${input.gsm ? input.gsm + "gsm" : ""}`.trim();
  } else if (materialType === "KAPTON") {
    const tapeLabel = KAPTON_TYPES.find((t) => t.value === input.tapeType)?.label || "Tape";
    name = `${tapeLabel} ${input.size || ""}`.trim();
  } else if (materialType === "GLUE") {
    name = `${input.glueType || ""} Glue ${input.weightKg ? input.weightKg + "kg" : ""}`.trim();
  } else if (materialType === "INK") {
    const color = resolveInkColor(input.inkColor, input.inkColorCustom);
    name = `${color || ""} Ink ${input.weightKg ? input.weightKg + "kg" : ""}`.trim();
  } else if (materialType === "ROPE") {
    name = `${input.ropeColor || ""} Rope ${input.ropeLengthM ? input.ropeLengthM + "m" : ""}`.trim();
  ...
  const barCode = materialType === "PAPER_ROLL"
  ...
  switch (materialType) {
    case "PAPER_ROLL":
      return {
        ...
        gsm: input.gsm || null,
        receivingDate: input.receivingDate ? new Date(input.receivingDate) : null,
        barCode,
      };
    case "GLUE":
      return {
...
export function materialToFormValues(material) {
  ...
  return {
    ...
    gsm: material.gsm ?? "",
    receivingDate: material.receivingDate ? new Date(material.receivingDate).toISOString().split("T")[0] : "",
...
export function getMaterialSummary(material) {
  switch (material.materialType) {
    case "PAPER_ROLL":
      return [
        material.paperType === "RECYCLED" ? "Recycled" : material.paperType === "VIRGIN" ? "Virgin" : null,
        material.paperColor === "BROWN" ? "Brown" : material.paperColor === "WHITE" ? "White" : null,
    ...
    case "KAPTON":
      return [
        KAPTON_TYPES.find((t) => t.value === material.tapeType)?.label || material.tapeType,
        material.size,
        material.unit,
      ]
        .filter(Boolean)
