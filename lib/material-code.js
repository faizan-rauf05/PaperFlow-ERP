...
export function buildMaterialRecord(input) {
  ...
  const base = {
    ...
    sheetCount: null,
    cartonSize: null,
    // Raw cost-price input passthrough — not Material columns themselves;
    // resolveCostPriceFields() (lib/cost-price.js) consumes these to derive
    // the actual costPrice* columns, then the caller deletes costPriceAmount
    // before persisting. costPriceCurrency/EntryBasis double as column names
    // too (the "as entered" snapshot), so they're intentionally left as-is.
    costPriceAmount: input.costPriceAmount ?? null,
    costPriceCurrency: input.costPriceCurrency || "KWD",
    costPriceEntryBasis: input.costPriceEntryBasis || "PER_UNIT",
  };
...
export function materialToFormValues(material) {
  ...
  return {
    ...
    costPriceAmount: material.costPriceOriginalAmount ?? "",
    costPriceCurrency: material.costPriceCurrency ?? "KWD",
    costPriceEntryBasis: material.costPriceEntryBasis ?? "PER_UNIT",
  };
}
